...
class FrameQ5 extends JFrame implements ActionListener {
	...
	public static boolean allGreater(ArrayList<Integer> list, int tar) {
		if(list.size() == 1)
			return list.get(0) > tar;

		int first = list.get(0);
		list.remove(0);

		return (first > tar) && allGreater(list, tar);
	}

	public FrameQ5(String s) {
		super(s);
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (target == b2) {
			...
			boolean res = allGreater(new ArrayList<Integer>(list), tar);
			t3.setText("" + res);
		}
	}
}