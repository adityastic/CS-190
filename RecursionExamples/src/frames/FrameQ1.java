...
class FrameQ1 extends JFrame implements ActionListener {
    ...
    public static int countOdd(ArrayList<Integer> arr) {
    	...
    	arr.remove(0);
        return ((firstEle % 2  == 1) ? 1 : 0) + countOdd(arr);
    }
 
    public FrameQ1(String s) {
        super(s);
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if (target == b1) {
            int v1 = sum(new ArrayList<Integer>(arr));
            t2.setText("" + v1);
        }


        if (target == b2) {
            int v1 = countOdd(new ArrayList<Integer>(arr));
            t3.setText("" + v1);
        }
    }
}